...
def get_auth_challenge():
    try:
        ...
        response = requests.post(url, json=payload, headers=headers, verify=kiq.crt)
...
def get_page_by_title(space_key, title):
    try:
        ...
        response = requests.get(url, headers=headers, auth=auth, params=params,verify=conf.crt)
...
def get_current_version(page_id):
    ...
    resp = requests.get(url, headers=headers, auth=auth,verify=conf.crt)
...
def create_page(space_key, title, content):
    ...
    try:
        response = requests.post(url, headers=headers, auth=auth, data=json.dumps(data),verify=conf.crt)
        if(response.status_code==200):
...
def attach_file(page_id, file_path, file_name):
    ...
    with open(file_path, "rb") as f:
        ...
        response = requests.post(upload_url, headers=headers_no_json, auth=auth, files=files,verify=conf.crt)
    try:
...
def update_page(page_id, title, html_content, current_version):
    try:
        ...
        response = requests.put(url, headers=headers, auth=auth, data=json.dumps(data),verify=conf.crt)
...
def get_tooltip_panel_content(external_id):
    try:
        ...
        def fetch_and_save_image(item_id):
            ...
            response = requests.get(url, headers=headers_1,verify=kiq.crt)
...
for itemid in data_itemid:
    ...
    response=requests.get(pagefetch_url,headers=headers_1,verify=kiq.crt)
    if response.status_code == 200:
        try:
            data = response.json()
            fields = data.get("fields", []) 
